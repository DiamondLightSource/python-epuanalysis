...
def inspect_xml():
    ...
    def browsexml():
        ...
        main_frame.filename = filedialog.askopenfilename(
            initialdir="~", title="Select EPU xml file"
        )
    ...
    def parsexml():
        ...
        ns[
            "s"
        ] = "http://schemas.datacontract.org/2004/07/Fei.Applications.Common.Services"
        ...
        # Exposure time
        time = root.find(
            "so:microscopeData/so:acquisition/so:camera/so:ExposureTime", ns
        ).text
        # Beam shift
        ...
        mag = root.find(
            "so:microscopeData/so:optics/so:TemMagnification/so:NominalMagnification",
            ns,
        ).text
        ...
        # Stage
        stagePositionA = root.find(
            "so:microscopeData/so:stage/so:Position/so:A", ns
        ).text
        stagePositionB = root.find(
            "so:microscopeData/so:stage/so:Position/so:B", ns
        ).text
        stagePositionX = root.find(
            "so:microscopeData/so:stage/so:Position/so:X", ns
        ).text
        stagePositionY = root.find(
            "so:microscopeData/so:stage/so:Position/so:Y", ns
        ).text
        stagePositionZ = root.find(
            "so:microscopeData/so:stage/so:Position/so:Z", ns
        ).text
