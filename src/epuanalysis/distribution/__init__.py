...
class ImageDistribution:
    ...
    def __sub__(self, other: ImageDistribution) -> ImageDistribution:
        if self._img.image != other._img.image:
            raise ValueError(
                f"To calculate the different of two ImageDistributions they must have the same base image: {self._img.image} != {other._img.image}"
            )
    ...
    def from_mrc(self, mrc_path: Path):
        ...
        if not len(data.shape) == 2:
            raise ValueError(
                f"mrc files passed to an ImageDistribution must have shape (n, m) not {data.shape}"
            )
    ...
    def from_particles(
        self, part_coords: List[Tuple[int, int]], data: List[float], diameter: float
    ):
        scale_factor = (
            self._img.xextent / self._img._detector_dimensions[0],
            self._img.yextent / self._img._detector_dimensions[1],
        )
